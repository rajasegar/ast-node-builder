...
function unaryExpression(node) {
  ...
  switch(argument.type) {
    case 'NumericLiteral':
      str = `j.unaryExpression('${operator}', ${numericLiteral(argument)}, ${prefix})`;
      break;
    case 'MemberExpression':
      str = `j.unaryExpression('${operator}', ${memberExpression(argument)}, ${prefix})`;
      break;
    default:
...
function templateElementValue(value) {
  let _cooked = value && value.cooked && value.cooked.replace('\n','\\n') || '';
...
function templateLiteral(node) {
  ...
  let _expressions = expressions.map(e => { 
    switch(e.type) {
      case 'Identifier':
        return identifier(e);
...
function classProperty(node) {
  ...
  if(decorators && decorators.length > 0) {
    ...
    node.decorators = [${decorators.map(d => decorator(d.expression)).join(',')}];
    return node;
    }())`;
  } else {
  str = `j.classProperty(
    ...
    switch(b.type) {
      ...
      case 'ClassMethod':
        ...
        ${(b.decorators && b.decorators.length > 0) 
          ? `node.decorators = [${b.decorators.map(d => decorator(d.expression)).join(',')}];`
          : '' 
        }
        return node;
        }())`;
...
function property(node) {
  let { key, value, computed, shorthand, decorators } = node;
  let str = '';
  switch(node.type) {
    case 'ObjectMethod':
      str = `j.objectMethod(
...
function buildAST(ast, wrapExpression = true) {
    ...
    let _ast = _body.map(node => {
      ...
      switch(node.type) {
        case 'VariableDeclaration':
          return wrapExpression ? variableDeclaration(node) : variableDeclarator(node.declarations[0]);
